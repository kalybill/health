@php
    use App\Models\{Nurse,Medical_record,Clients};
@endphp
@extends('layouts.app')

@section('page_header')
    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
        Outstanding Note/RS Reports (Pharmacy)
    </h1>
@endsection
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select name="id" id="" class="form-select" data-control="select2">
                            ...
                            @foreach (Medical_record::where('client', '!=', null)->get() as $item)
                            <option value="{{ $item->client }}">{{ Clients::find($item->client)->cname }}</option>
                            @endforeach
                        </select>
                    </div>
...
@endsection
@section('scripts')
<script src="{{ asset('assets/js/self/out_by_phama.js') . env('VERSION') }}"></script>
<script>
