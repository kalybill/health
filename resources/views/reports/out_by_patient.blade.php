@php
    use App\Models\{Nurse,Medical_record,Referral};
@endphp
@extends('layouts.app')
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select name="id" id="" class="form-select" data-control="select2">
                            ...
                            @foreach (Medical_record::where('patient_id', '!=', null)->get() as $item)
                            <option value="{{ $item->patient_id }}">{{ Referral::find($item->patient_id)->name }}</option>
                            @endforeach
                        </select>
                    </div>
...
@endsection
@section('scripts')
<script src="{{ asset('assets/js/self/out_by_patient.js') . env('VERSION') }}"></script>
<script>
    $('.subNavMDRPT').addClass('hover show');
    $('#navOutAllPatient').addClass('active')
</script>
@endsection
